...
class LitModel(pl.LightningModule):
    ...
    def log_tb_images(self, viz_batch) -> None:
        ...
        for img_idx, (image, y_true, y_pred, batch_idx) in enumerate(zip(*viz_batch)):
            ...
            plt.title("GT vs Pred National GSP Single Shot")
            plt.legend(loc="best")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--steps", type=int, default=70, help="Number of forecast steps per pass")
    ...
    model_checkpoint = ModelCheckpoint(
        ...
        save_last=True,
        save_top_k=10,
        dirpath=f"./metnet_gsp_single_shot_nmae_relu_35hour_inchannels{input_channels}"
                f"_step{args.steps}"
                f"_size{args.size}"
                f"_sun{args.sun}"
                f"_sat{args.sat}"
