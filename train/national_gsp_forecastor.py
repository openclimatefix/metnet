...
class LitModel(pl.LightningModule):
    def __init__(
        ...
        self.model = MetNetPV(
            ...
            num_pv_systems=num_systems,
            pv_fc_out_channels=pv_out_channels,
            pv_id_embedding_channels=pv_id_channels,
            num_pv_embeddings=num_embed
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i, f in enumerate(
            ...
            out = self(x, pv_yield, pv_id, fs[i]-1)
            truths = y[:,fs[i], 0]
...
if __name__ == "__main__":
    ...
    model_checkpoint = ModelCheckpoint(
        ...
        dirpath=f"/mnt/storage_ssd_4tb/metnet_models/metnet_gsp_inchannels{input_channels}"
                f"_step{args.steps}"
                f"_size{args.size}"
                f"_sun{args.sun}"
                f"_sat{args.sat}"
    ...
    model = LitModel(
        ...
        num_embed=args.num_embedding_size,
        num_systems=args.num_pv_systems,
