...
def temporal_concatenation(
    data, dates, targets, target_dates, concat=7, overlap=0, spaced=3, lead_times=60
):
    ...
    Y_dates = []
    for i, j in zip(
        range(0, x_limit, concat - overlap),
        range(concat * spaced - 2, n_y, (concat - overlap) * spaced),
    ):
        ...
        try:
            ...
            fiver = [
                temp_dates[-1],
                temp_dates_target[0],
            ]  # final X date and first Y date should be 5 spaced minutes
...
def datetime_encoder(data, dates, plotter=False):
    ...
    periodicals = [
        np.sin(2 * np.pi * year_days / 365, dtype=data_type),
...
def load_data(
    ...
    X, Y, X_dates, Y_dates = temporal_concatenation(
        data,
        dates,
        Y,
        Y_dates,
...
def rain_binned(Y, n_bins=51, increment=2):
    ...
    for i in range(n_bins - 1):
        ...
        rain_bin = np.zeros(
            (n, leads, w, h)
        )  # Y.shape = (None,lead_times, bin_channel, width/4,heigth/4)
...
if __name__ == "__main__":
    ...
    data, dates = load_data(
        "combination_all_pn157.h5", N=500, downsample=True, spacedepth=True, printer=True
    )
    print(data.nbytes)
