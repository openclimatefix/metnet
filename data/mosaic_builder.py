...
def load_satellite_data(store_url: str, time: Optional[str] = None) -> xr.Dataset:
    ...
    if time:
        ...
        if not (np.all(time_diffs > 0) or np.all(time_diffs < 0)):
            print("   ⚠️  Time index is not monotonic, sorting...")
            # Sort by time
...
def create_mosaic(
    satellite_urls: List[str], time: str, output_path: Optional[Path] = None
) -> xr.Dataset:
...
def create_global_mosaic(
    ...
    for i, url in enumerate(satellite_urls):
        ...
        try:
            ...
            print(
                f"   📍 Satellite longitude: {sat_lon}°, height: {sat_height/1e6:.1f}k km, sweep: {sat_sweep}"
            )
            ...
            for band in bands:
                ...
                reprojected = _reproject_geostationary(
                    sat_data[band], target_lons, target_lats, sat_lon, sat_height=sat_height
                )
                ...
                print(
                    f"   ✅ Processed band {band} in {band_total:.2f}s (reproject: {reproject_time:.2f}s, weight: {weight_time:.2f}s, blend: {blend_time:.2f}s)"
                )
    ...
    mosaic_ds = xr.Dataset(
        data_vars={band: (["lat", "lon"], mosaic_data[band]) for band in mosaic_data},
        coords={"lat": target_lats, "lon": target_lons, "time": target_time},
        attrs={
...
def _reproject_geostationary(
    ...
    try:
        ...
        if "x_geostationary" not in data_array.coords or "y_geostationary" not in data_array.coords:
            print("    ⚠️  Missing geostationary coordinates")
            return None
        ...
        target_proj_dict = {"proj": "longlat", "datum": "WGS84"}
...
def _calculate_azimuth_weights(
    lons: np.ndarray, lats: np.ndarray, sat_lon: float, sat_lat: float = 0.0
) -> np.ndarray:
...
def test_global_mosaic():
    ...
    satellite_urls = [SATELLITE_CONFIG["goes-east"]["url"], SATELLITE_CONFIG["goes-west"]["url"]]
    ...
    print("   Visible: C01, C02, C03")
    print("   Near-IR: C04, C05, C06")
    print("   IR-SW: C07, C08, C09, C10")
    print("   IR-LW: C11, C12, C13, C14, C15, C16")
    print("   Extra: C17, HRV")
    ...
    try:
        ...
        for band in sorted(mosaic.data_vars):
            ...
            if valid_pixels > 0:
                print(
                    f"   {band}: {coverage:5.1f}% coverage, range [{np.nanmin(data):8.3f}, {np.nanmax(data):8.3f}]"
                )
            else:
...
def main():
    ...
    mosaic_parser.add_argument(
        "--time", required=True, help="Time to create mosaic for (ISO format)"
    )
    mosaic_parser.add_argument("--output", type=Path, help="Output path for mosaic")

    # Test command
    subparsers.add_parser("test-mosaic", help="Test global mosaic creation")
    ...
    if args.command == "inspect":
        ...
        create_global_mosaic(args.stores, target_time, output_path=args.output)
