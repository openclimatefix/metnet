...
def longlatencoding(data):
    ...
    with h5.File("lonlatelev_full.h5","r") as FF:
        lonlatelev = FF["lonlatelev"]
        lonlatelev = np.array(lonlatelev)
        print("LONLAT SHAPE:", lonlatelev.shape)
...
def rain_binned(Y, n_bins = 51,increment = 0.2, x = None):
    SHAPE = Y.shape
    n,leads,w,h = SHAPE
    ...
    '''for i in range(n):
        ...
        ax[1].set_title("y")
        plt.show()'''
    rain_bins = np.zeros((n,leads,n_bins,w,h), dtype = np.int8)
    counter = []
    for i in range(n_bins-1):
        if i%100==0: print(f"Bin progress:",i)
        bin_min = i*increment
...
def prepare_files(h5_path = "data3_500k.h5",lead_times = 60, concat = 7,  square = (0,448,881-448,881), downsampling_rate = 2, overlap = 0, spaced=3,downsample = True, spacedepth =True,centercrop=True,box=2,printer=True, rain_step = 0.2, n_bins=512):
    #15 minutes between datapoints is default --> spaced = 3
    ...
    N = 500000
    partial = 400000
    for start in range(partial,partial+100000,batch_size):
        print(f"-------------------- STARTING AT {start} --------------------")
        snapshots = []
        dates = []
        all_snapshots = []
        all_dates = []
        for i, array,date in h5_iterator(h5_path, batch_size, starter=start):
        ...
        if length_x==length_y:
            ...
            Y_lim_x = slice(center_x-length_x//2,center_x+length_x//2)
            Y_lim_y = slice(center_x-length_x//2,y1-(center_x-length_x//2))
        print("SLICED Y in x: ",Y_lim_x)
        print("SLICED Y in y: ",Y_lim_y)
        Y = all_data[:,Y_lim_y,Y_lim_x]
        del(all_data) #MANAGE MEMORY
        print(f"\nY shape here (not ready): {Y.shape}")
        ...
        data = np.swapaxes(np.swapaxes(data,3,1),2,3)
        print(f"\nData swapping axes to get channel first, now shape: {data.shape}")
        X,Y, X_dates,Y_dates = temporal_concatenation(data,dates,Y,all_dates,concat = concat, overlap = overlap, spaced = spaced,lead_times = lead_times)
        ...
        idx_to_remove = np.where(to_mean > 70)
        to_mean[idx_to_remove] = 0
        means = np.mean(to_mean, axis=(1,2,3,4))
        ...
        #print(f"\nOnly keeping {to_keep} out of {N} samples to reduce low rainfall events. New X shape: {X.shape}")
        path_loc = "/proj/berzelius-2022-18/users/sm_valfa/metnet_pylight/metnet/data_moving_frame/"
        for i in range(X.shape[0]):
            mean_str = str(means[i])
            mean_str = mean_str[0:max(5,len(mean_str))]
            start_date = X_dates[i][0]
            full_path_X = path_loc + mean_str +"_" + start_date +"_"+"X"+ ".npy"
            full_path_Y = path_loc + mean_str +"_" + start_date +"_"+"Y"+ ".npy"
            np.save(full_path_X,X[i])
            np.save(full_path_Y,Y[i])
...
def prepare_batches(batch_size = 8):
    ...
    names_sorted = [names[idx] for idx in idx_sorted]
    means_sorted = means[idx_sorted]
    dates_sorted = [dates[idx] for idx in idx_sorted]
    print("MEANS: ", means[idx_sorted])
    print(len(means))
    N = len(means)
    
    batches_path = f"/proj/berzelius-2022-18/users/sm_valfa/metnet_pylight/metnet/batch_{batch_size}/"
    paths = {}
    for ID in ["train", "val", "test"]:
        #os.makedirs(batches_path+ID + "/")
        paths[ID] = batches_path + ID + "/"
    train, val, test = (10, 2, 2)
    leng = sum([train, val, test])
    
    for i in range(0,N//batch_size):
        if i%10==0:
            print(f"batch {i} / {N//batch_size}")
        X = np.empty((batch_size, 7, 15, 112,112))
        Y = np.empty((batch_size, 60, 128, 28,28))
        
        if 0 <= i%leng < train: ID = "train"
        elif train <= i%leng < train+val: ID = "val"
        elif train+val <= i%leng: ID = "test"
        
        batch_mean = np.mean(means_sorted[i*batch_size:(i+1)*batch_size])
        batch_names_X = [data_path + file_name for file_name in names_sorted[i*batch_size:(i+1)*batch_size]]
        batch_names_Y = [file_name.replace("X","Y") for file_name in batch_names_X]
        
        for k,(x,y) in enumerate(zip(batch_names_X,batch_names_Y)):
            X[k] = np.load(x)
            Y[k] = np.load(y)
            
        np.save(paths[ID]+str(batch_mean)[0:5]+ "_" +dates_sorted[i*batch_size]+"_X.npy",X)
        np.save(paths[ID]+str(batch_mean)[0:5]+ "_" +dates_sorted[i*batch_size]+"_Y.npy",Y)

def prepare_new_mean_split():
    data_path = "/proj/berzelius-2022-18/users/sm_valfa/metnet_pylight/metnet/data_exclusive/"
    file_list = os.listdir(data_path)
    non_zeros = np.zeros((128,))
    dates = []
    names = [] 
    for k,name in enumerate(file_list):
        if k%100 == 0: print(f"{k} / {len(file_list)} samples meaned!")
        if not name[-4:] == ".npy": continue
        if name[-5:] == "Y.npy": continue
        Y_name = name.replace("X","Y")
        Y_here = np.load(data_path+Y_name) #Y.shape = (60, 128, 28, 28)
        
        non_zero = np.sum(Y_here[:,1:,:,:], axis = (0,1,2,3))
    ...
    idx_sorted = np.argsort(-non_zeros)
    np.save("/proj/berzelius-2022-18/users/sm_valfa/metnet_pylight/metnet/no_zeros.npy",non_zeros)
    
    names_sorted = [names[idx] for idx in idx_sorted]
    non_zeros_sorted = non_zeros[idx_sorted]
    dates_sorted = [dates[idx] for idx in idx_sorted]
    
    print("MEANS: ", non_zeros[idx_sorted])
    print(len(non_zeros))
    N = len(non_zeros)
    
    y_sort_path = f"/proj/berzelius-2022-18/users/sm_valfa/metnet_pylight/metnet/bin_sorted_data/"
    paths = {}
    for ID in ["train", "val", "test"]:
        #os.makedirs(y_sort_path+ID + "/")
        paths[ID] = y_sort_path + ID + "/"
    train, val, test = (10, 2, 2)
    leng = sum([train, val, test])
    
    for i in range(0,N):
        if i%100==0:
            print(f" {i} / {N} samples done!")
        ...
        X_name = data_path + names_sorted[i]
        ...
        np.save(paths[ID]+str(non_zeros_sorted[i])+ "_" +dates_sorted[i]+"_X.npy",X)
        np.save(paths[ID]+str(non_zeros_sorted[i])+ "_" +dates_sorted[i]+"_Y.npy",Y)
        
def plot_rain_distribution(h5_path = "data3_500k.h5", maxN = 1000):
    mean_rain_month = [[] for i in range(12)]
    
    mean_rain_hour = [[] for i in range(24)]
    
    for i, array,date in h5_iterator(h5_path, maxN = maxN):
        
        if (i+1)%1000==0:
            print("Counted samples: ", i+1)
            monthly_mean = np.array([np.mean(month) for month in mean_rain_month])
            hourly_mean = np.array([np.mean(hour) for hour in mean_rain_hour])
            print(monthly_mean)
            print(hourly_mean)
            #np.save("monthly_mean.npy", monthly_mean)
            #np.save("hourly_mean.npy", hourly_mean)
            
            fig, axs = plt.subplots(1,2, subplot_kw={'projection': 'polar'})
            equals = np.linspace(0, 360, 24, endpoint=False) 
           
            axs[0].scatter(np.deg2rad(equals), hourly_mean)       

            # Set the circumference labels
            axs[0].set_xticks(np.linspace(0, 2*np.pi, 24, endpoint=False))
            ...
            equals = np.linspace(0, 360, 12, endpoint=False) #np.arange(24)
            #ones = np.ones(24)
            axs[1].scatter(np.deg2rad(equals), monthly_mean)       
            ...
            axs[0].set_title("Hourly rain rates")
            axs[1].set_title("Monthly rain rates")
            plt.savefig("yearly_rates_plots")
            plt.show()
        listed = date.split("_")
        y, m, d, hour, minute =  [int(a) for a in listed]
        
        array = 0.4*array - 30
        rain = (10**(array / 10.0) / 200.0)**(1.0 / 1.6)
        
        idx = np.where(array<70)
        
        if len(idx)==0: continue
        mean_rain = np.mean(rain[idx])
        mean_rain_month[m].append(mean_rain)
        mean_rain_hour[hour].append(mean_rain)
    
    monthly_mean = [np.mean(month) for month in mean_rain_month]
    hourly_mean = [np.mean(hour) for hour in mean_rain_hour]
    print(hourly_mean)
    np.save("monthly_mean.npy", monthly_mean)
    np.save("hourly_mean.npy", hourly_mean)
    ...
    ones = np.ones(24)
    ax.scatter(np.deg2rad(equals), hourly_mean)       
    ...
    equals = np.linspace(0, 360, 12, endpoint=False) #np.arange(24)
    #ones = np.ones(24)
    ax.scatter(np.deg2rad(equals), monthly)       
...
if __name__=="__main__":
    plot_rain_distribution(maxN = None)
    square = (0,448,0,880)
    #prepare_files(square=square)
    #prepare_new_mean_split()
